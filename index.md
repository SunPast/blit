...
To run the command, just pass the `command` object to the
...
is provided for completeness. Instead of using this function, we
recommend using
...
can add `conda`/`mamba` environment prefix to the `PATH` environment
variable.
...
[`cmd_condaenv()`](https://wanglabcsu.github.io/blit/reference/cmd_wd.md)
function accepts multiple `conda`/`mamba` environment prefixes and an
optional `root` argument specifying the path to the `conda`/`mamba` root
prefix. If `root` is not provided, the function searches for the root in
the following order:
...
3.  the root prefix of
    ...
    (Please see the `Software management` section for details).

The
[`cmd_condaenv()`](https://wanglabcsu.github.io/blit/reference/cmd_wd.md)
function searches for the specified environment prefix within the
provided `root` path.
...
You can install `micromamba` with
...
The
[`appmamba()`](https://wanglabcsu.github.io/blit/reference/appmamba.md)
function executes specified `micromamba` commands. Running it without
arguments shows the help document:
...
For these commands, you can also use
[`cmd_help()`](https://wanglabcsu.github.io/blit/reference/cmd_run.md)
to print the help document.
